...
def configure_cmake():
    print("configuring compilation")
    subprocess.run("cmake -B build -DCMAKE_CXX_COMPILER=g++-14 -G Ninja -DCMAKE_BUILD_TYPE=Release".split(), stdout=subprocess.PIPE)

def get_benchmark_runner_executable():
    # the current cwd needs to be the root of dqsim
    print("compiling benchmark runner")
    subprocess.run("cmake --build build --parallel --target ds_bench_s".split(), stdout=subprocess.PIPE)

def get_simulator_executable():
    # the current cwd needs to be the root of dqsim
    print("compiling simulator")
    subprocess.run("cmake --build build --parallel --target ds_sim".split(), stdout=subprocess.PIPE)

def extract_zip(zip_path, extract_subdir, dest_root):
    temp_dir = Path(tempfile.mkdtemp())
    with zipfile.ZipFile(zip_path, "r") as zf:
        zf.extractall(temp_dir)

    src = temp_dir / extract_subdir
